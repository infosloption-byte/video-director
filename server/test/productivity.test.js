...
test("M15 templates support creation, ownership, application, and deletion", () => {
  ...
  route('router\\.post\\("/:id/apply-template/:templateId"');
  assert.match(source, /if \(!name\) return res\.status\(400\)/);
  assert.match(source, /where: \{ id: req\.params\.templateId, userId: req\.user\.id \}/);
  assert.match(source, /function applyTemplateToProject\(template, project\)/);
  assert.match(source, /compatibleAsset\?\.scene\?\.id === targetScene\.id/);
  assert.match(source, /expectedVersion !== project\.editor\.version/);
  assert.match(source, /"template\.created"/);
  assert.match(source, /"template\.applied"/);
  assert.match(source, /res\.status\(204\)\.end\(\)/);
});

test("M15 review links support listing, creation, public reads, expiry, revocation and comments", () => {
  route('router\\.get\\("/:id/review-links"');
  ...
  assert.match(source, /Number\.isNaN\(expiresAt\.getTime\(\)\)/);
  ...
  assert.match(source, /"review_link\.created"/);
  assert.match(source, /"review_link\.revoked"/);
});

test("M15 activity history is recorded for version, duplication, template, review, and AI actions", () => {
...
test("M15 project-scoped APIs consistently require the authenticated project owner", () => {
  const projectScopedHandlers = [
    ...
    'router\\.post\\("/:id/apply-template/:templateId"',
    'router\\.get\\("/:id/review-links"',
