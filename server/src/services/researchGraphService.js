...
function sourcePublisher(source = {}) { return source.publisher || source.source_name || source.sourceName || null; }
function sourceUrl(source = {}) { return source.url || source.sourceUrl || null; }
...
function scoreSource(source, quality = {}) {
  ...
  const evidence = clamp(quality.evidence_quality ?? quality.evidence ?? (source.read_status === "read" ? 80 : 15), 0);
...
function buildEvidence(brief, sourceRecords) {
  ...
  previews.forEach((preview, index) => {
    ...
    rows.push({ sessionId: source.sessionId, sourceId: source.id, evidenceIndex: index, passageText: passage,
...
export async function persistResearchGraph(projectId, brief, { status = "completed" } = {}) {
  ...
  for (let index = 0; index < sources.length; index += 1) {
    const source = sources[index] || {}; const sourceIndex = Number.isInteger(source.index) ? source.index : index; const url = sourceUrl(source);
    if (!url) continue;
    const quality = qualityByIndex.get(sourceIndex) || {}; const scores = scoreSource(source, quality);
    const excerpt = source.read_excerpt || source.readExcerpt || null;
    const record = await prisma.researchSource.create({ data: { sessionId: session.id, sourceIndex, url: String(url),
      canonicalUrl: source.canonical_url || source.canonicalUrl || null, title: source.title || null, publisher: sourcePublisher(source),
      sourceClass: source.source_class || source.sourceClass || null, reliability: source.source_reliability || source.reliability || null,
      publishedAt: parseDate(source.published_at || source.publishedAt), retrievedAt: parseDate(source.retrieved_at || source.retrievedAt) || new Date(),
      readStatus: source.read_status || source.readStatus || "unread", readExcerpt: excerpt ? String(excerpt).slice(0, 12000) : null,
      contentHash: hashContent(source.read_content || source.content || excerpt), authorityScore: scores.authority, relevanceScore: scores.relevance,
      evidenceScore: scores.evidence, recencyScore: scores.recency, independenceScore: scores.independence, transparencyScore: scores.transparency } });
    sourceRecords.push(record);
  }

  const evidenceRecords = [];
  for (const row of buildEvidence(brief, sourceRecords)) evidenceRecords.push(await prisma.researchEvidence.create({ data: row }));
  ...
  for (let index = 0; index < claims.length; index += 1) {
    ...
    const record = await prisma.researchClaim.create({ data: { sessionId: session.id, claimIndex: index,
      claimText: String(claim.claim || claim.claimText || "").trim(), evidenceLevel: claim.evidence_level || claim.evidenceLevel || null,
      modelConfidence: clamp(claim.model_confidence ?? claim.confidence, 0), verifiedConfidence: clamp(claim.verified_confidence ?? claim.confidence, 0),
    ...
    if (!evidenceIndexes.length && sourceIndexes.length) {
      const matchingEvidence = evidenceRecords.filter((evidence) => sourceIndexes.map(Number).includes(sourceRecords.find((source) => source.id === evidence.sourceId)?.sourceIndex));
    ...
    const sourceQuality = sourceIndexes.map(Number).map((sourceIndex) => qualityByIndex.get(sourceIndex)).filter(Boolean);
    const average = (field) => sourceQuality.length ? clamp(sourceQuality.reduce((sum, item) => sum + clamp(item[field]), 0) / sourceQuality.length) : 0;
    const claimVerification = verification.claims?.[index] || {};
    const contradiction = (verification.conflicts || []).some((conflict) => Array.isArray(conflict.finding_indexes) && conflict.finding_indexes.includes(index)) ? 70 : 0;
    await prisma.researchVerification.create({ data: { claimId: record.id,
      corroborationScore: clamp(claimVerification.corroboration_score), contradictionScore: contradiction,
      authorityScore: clamp(claimVerification.authority_score ?? average("authority")), relevanceScore: average("relevance"),
      evidenceQualityScore: average("evidence_quality") || average("evidence"), recencyScore: average("recency"),
      independenceScore: clamp(claimVerification.independent_domains ? Math.min(100, claimVerification.independent_domains * 50) : average("independence")),
      transparencyScore: average("transparency"), traceable: Boolean(claimVerification.traceable), notes: claimVerification.verification_status || null } });
  }

  for (const conflict of Array.isArray(verification.conflicts) ? verification.conflicts : []) {
