...
function parseJson(text) {
  ...
  if (firstBrace >= 0 && lastBrace > firstBrace) {
    try { return JSON.parse(raw.slice(firstBrace, lastBrace + 1)); } catch { /* normalize below */ }
  }
...
async function callGemini(prompt) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
    try {
      ...
      if (!text) { const error = new Error(`Gemini returned no storyboard JSON${candidate?.finishReason ? ` (finish reason: ${candidate.finishReason})` : ""}.`); error.status = 502; throw error; }
...
function fallbackSentences(project, signal) {
  const bullets = String(project.researchSummary || "").split(/\n+/).map((line) => line.replace(/^\s*[•*-]\s*/, "").trim()).filter(Boolean);
  ...
  return [`Here is what is changing: ${signal.title}.`, mechanism, ...facts, "The practical takeaway is what this means next for people using the technology."].filter(Boolean);
}
function buildFallbackStoryboard({ project, signal, length }) {
  ...
  return lines.map((spokenText, index) => normalizeScene({ title: index === 0 ? "The signal" : index === lines.length - 1 ? "What happens next" : `The mechanism ${index}`, spoken_text: spokenText, duration_seconds: base, why_line: "Fallback storyboard created from the completed research brief while Gemini is rate-limited.", why_picture: "Use a concrete visual tied to the mechanism or claim described in this scene.", broll_search_term: signal.title }, index, length));
}
...
export async function generateStoryboard({ project, signal, researchCorpus = null }) {
  ...
  const prompt = `You are Helix, a short-form science and technology Reel director. Create a complete scene-by-scene storyboard for a ${length}-second vertical video. Use the persisted research corpus as the factual authority. Never invent facts, and never present an unverified claim as established. When evidence conflicts, preserve the uncertainty instead of averaging it away. Keep the selected framework and tone. Each scene needs a concise spoken line, a reason that line belongs, a reason for the picture, and a concrete B-roll search phrase suitable for Pexels.
