...
function buildStrengthAnswer(corpus) {
  ...
  const top = rankedEvidence.slice(0, 3).map(({ item, authority, hasSource }, index) => { const source = sourceMap.get(item.sourceId); const attribution = hasSource && source?.title ? ` — ${source.title}` : " — persisted research"; return `${index + 1}. ${item.passageText}${attribution}${hasSource && authority ? ` (authority ${Math.round(authority)})` : ""}`; }).join("\n");
