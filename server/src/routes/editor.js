...
function checkAiRateLimit(userId) {
  ...
  const recent = (aiRequestBuckets.get(key) || []).filter((timestamp) => now - timestamp < AI_WINDOW_MS);
  if (recent.length >= AI_REQUEST_LIMIT) return false;
  recent.push(now);
  aiRequestBuckets.set(key, recent);
  if (aiRequestBuckets.size > 1000) for (const [bucketKey, timestamps] of aiRequestBuckets) if (!timestamps.some((timestamp) => now - timestamp < AI_WINDOW_MS)) aiRequestBuckets.delete(bucketKey);
  return true;
}
async function logAiActivity(projectId, userId, action, metadata = {}) {
