...
function normalizeTransition(value, clipDuration) {
  ...
  return { preset, duration: clamp(toNumber(value?.duration, 0.35), 0.05, maxDuration) };
}

function normalizeEffect(value) {
  const preset = ALLOWED_EFFECTS.has(String(value?.preset)) ? String(value.preset) : "none";
  return { preset, intensity: clamp(toNumber(value?.intensity, 0.5), 0, 1) };
}
...
function buildInitialTimeline(scenes) {
  ...
  for (const scene of scenes) {
    ...
    captions.push({ id: `caption-${scene.id}`, type: "caption", sourceId: scene.id, start: cursor, duration, text: scene.spokenText, style: "default", position: "lower-middle" });
    cursor += duration;
  }
  return { schemaVersion: 1, fps: 30, width: 1080, height: 1920, duration: Number(cursor.toFixed(3)), tracks: [
...
function sanitizeTimeline(input, fallback) {
  ...
  const safeTracks = tracks.map((track, trackIndex) => {
    ...
    return { id: trackId, kind, name: String(track.name || fallback.tracks[trackIndex]?.name || "Track"), locked: Boolean(track.locked), muted: Boolean(track.muted), clips: Array.isArray(track.clips) ? track.clips.map((clip, index) => sanitizeClip(clip, index, kind, trackId)) : [] };
  });
...
router.patch("/:id/editor", async (req, res) => {
  try {
    ...
    if (current && expectedVersion != null && expectedVersion !== current.version) return res.status(409).json({ error: "Editor changed elsewhere. Reload before saving.", version: current.version, timeline: current.timeline });
    const timeline = sanitizeTimeline(req.body?.timeline, fallback);
    const editor = current ? await prisma.projectEditor.update({ where: { projectId: project.id }, data: { version: { increment: 1 }, timeline } }) : await prisma.projectEditor.create({ data: { projectId: project.id, version: 1, timeline } });
...
// M14 accepts only structured, reversible operations. Preview never persists; apply persists only ProjectEditor.timeline.
router.post("/:id/editor/ai/preview", async (req, res) => {
  try {
    const result = await getEditor(req.params.id);
    if (!result) return res.status(404).json({ error: "Project not found." });
    const operations = validateEditorOperations(req.body?.operations);
    const preview = previewEditorOperations(result.editor.timeline, operations);
    res.json({ baseVersion: result.editor.version, operations: preview.operations, timeline: preview.timeline, persisted: false });
  } catch (error) {
    res.status(400).json({ error: error.message || "Invalid editor operations." });
  }
});

router.post("/:id/editor/ai/apply", async (req, res) => {
  try {
    const result = await getEditor(req.params.id);
    if (!result) return res.status(404).json({ error: "Project not found." });
    const expectedVersion = Number(req.body?.version);
    if (!Number.isInteger(expectedVersion) || expectedVersion !== result.editor.version) return res.status(409).json({ error: "Editor changed elsewhere. Reload before applying AI edits.", version: result.editor.version, timeline: result.editor.timeline });
    const operations = validateEditorOperations(req.body?.operations);
    const timeline = applyEditorOperations(result.editor.timeline, operations);
    const editor = await prisma.projectEditor.update({ where: { projectId: result.project.id }, data: { version: { increment: 1 }, timeline } });
    res.json({ editor: { id: editor.id, projectId: editor.projectId, version: editor.version, timeline: editor.timeline, updatedAt: editor.updatedAt }, operations, persisted: true });
  } catch (error) {
    res.status(400).json({ error: error.message || "Unable to apply AI edits." });
  }
});
