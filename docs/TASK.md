...
## M11 — Advanced Video Editor Core
**Status:** Not started

Goal: add a dedicated professional editing workspace on top of the existing
Storyboard + narration + asset data, while keeping Storyboard intact.

- [ ] Add `EditorPage.jsx` and a clear Storyboard → Edit video entry
...
- [ ] Replace selected B-roll with another existing scene asset
- [ ] Adjust clip duration without mutating the original media file
- [ ] Audio volume, mute, and basic fade controls
- [ ] Caption text/style/position/emphasis controls
- [ ] Basic transition presets that are safe for Remotion rendering
...
- [ ] Keep preview and render driven from the same canonical timeline model

**Acceptance:** a user can open an existing completed storyboard, make several
edits, preview them, refresh without losing changes, and render a video whose
timing/visuals/captions match the editor exactly.
...
Goal: make the editor timeline the single source of truth for final rendering.
...
- [ ] Prevent stale renders from being shown as the current edit
...
**Acceptance:** changing the editor timeline changes the final MP4 exactly as
expected, with no preview/render drift.
...
Goal: use Gemini to suggest reversible edits to the editor model rather than
blindly rewriting rendered video.
...
M9 Facebook production → DEFERRED / separate decision
```
...
- `2026-08-28` — Approved next-level direction: add real user accounts/authentication and a dedicated advanced video editor on top of the existing Storyboard/narration/render pipeline.
- `2026-08-28` — Editor architecture is non-destructive: immutable media + canonical timeline JSON/EDL-like state + Remotion as final renderer source of truth.
- `2026-08-28` — Add a reusable media-library layer before expanding editor/AI functionality so user uploads and generated/external assets share consistent ownership and lifecycle rules.
