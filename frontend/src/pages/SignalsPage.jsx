...
function toCardSignal(row, index) {
  return { ...row, id: row.id, origin: row.origin || "suggested", sourceType: row.sourceType, sourceReliability: row.sourceReliability, searchQuery: row.searchQuery, sourceUrl: row.sourceUrl, rawContent: row.rawContent, rank: String(row.rank ?? index + 1).padStart(2, "0"), category: row.category, pct: row.heatPct ?? "", title: row.title, description: row.description ?? "", whyLabel: "WHY THIS", why: row.whyReasoning, source: row.sourceName ?? "", sourceNote: RELIABILITY_LABEL[row.sourceReliability] ?? "", thumb: swatchSets[SWATCH_KEYS[index % SWATCH_KEYS.length]][0] };
}
...
export default function SignalsPage() {
  ...
  const headerActions = (
    <>
      ...
      <button type="button" className="btn btn-ghost hx-header__compact-action hx-scan-action" aria-label="Scan signals" title="Scan signals"><IconScan className="btn-icon" /><span className="hx-header__action-label">Scan signals</span></button>
    </>
  );
  ...
  return (
    <div className="hx-page hx-landing">
      ...
      <section className="container hx-topic-research" aria-labelledby="topic-research-title">
        <div className="hx-topic-research__copy"><p className="eyebrow">Research with Helix</p><h2 id="topic-research-title">Have your own story in mind?</h2><p>Start with a question instead of a platform signal. Helix will search, read, compare, and verify evidence, then turn the conversation into the same research brief used by Setup and Storyboard.</p></div>
