  ...
  return <div className="hx-page advanced-editor-shell">
    ...
    <main className="container advanced-editor">
      <header className="advanced-editor__header">
        ...
        <div className="advanced-editor__actions"><span className="editor-save-state">{saving ? "Saving…" : message || `Version ${version}`}</span><Link className="btn btn-ghost" to={`/editor/${id}/ai`}>AI Assistant</Link><button className="btn btn-ghost" onClick={undo} disabled={!history.length}>Undo</button><button className="btn btn-ghost" onClick={redo} disabled={!future.length}>Redo</button><button className="btn btn-cream" onClick={() => void saveTimeline()} disabled={saving || !dirty}>{saving ? "Saving…" : "Save changes"}</button><Link className="btn btn-ghost" to={`/editor/${id}/render`}>Render MP4</Link></div>
      </header>
      ...
      <section className="advanced-editor__topgrid">
        ...
        <aside className="advanced-editor__inspector">
          ...
          {selected && <div className="editor-form">
            ...
            {selected.trackId === "video" && selectedScene?.assets?.length > 0 && <label>Visual<SelectMenu value={selected.assetId || ""} options={[{ value: "", label: "Current" }, ...selectedScene.assets.map((asset) => ({ value: asset.id, label: `Visual ${asset.sortOrder + 1}` }))]} onChange={(value) => replaceVisual(selectedScene.assets.find((asset) => asset.id === value))} ariaLabel="Visual" /></label>}
            {selected.trackId === "video" && <div className="editor-form__panel"><div className="editor-form__subheading">Transitions</div><div className="editor-form__split"><label>In<SelectMenu value={selectedTransitionIn.preset} options={TRANSITION_PRESETS} onChange={(value) => updateSelectedTransition("transitionIn", "preset", value)} ariaLabel="Transition in" /></label><label>Out<SelectMenu value={selectedTransitionOut.preset} options={TRANSITION_PRESETS} onChange={(value) => updateSelectedTransition("transitionOut", "preset", value)} ariaLabel="Transition out" /></label></div><div className="editor-form__split"><label>In duration<input type="number" min="0.05" max="1.5" step="0.05" value={selectedTransitionIn.duration} onChange={(event) => updateSelectedTransition("transitionIn", "duration", event.target.value)} /></label><label>Out duration<input type="number" min="0.05" max="1.5" step="0.05" value={selectedTransitionOut.duration} onChange={(event) => updateSelectedTransition("transitionOut", "duration", event.target.value)} /></label></div></div>}
            {selected.trackId === "video" && <div className="editor-form__panel"><div className="editor-form__subheading">Effect preset</div><label>Motion<SelectMenu value={selectedEffect.preset} options={EFFECT_PRESETS} onChange={(value) => updateSelectedEffect("effect", value)} ariaLabel="Motion" /></label><label>Intensity<input type="range" min="0" max="1" step="0.05" value={selectedEffect.intensity} onChange={(event) => updateSelectedEffect("intensity", event.target.value)} /><span className="field-readout">{Math.round(selectedEffect.intensity * 100)}%</span></label></div>}
            ...
            {selected.type === "caption" && <><label>Caption text<textarea rows="4" value={selected.text || ""} onChange={(event) => updateSelected("text", event.target.value)} /></label><div className="editor-form__split"><label>Position<SelectMenu value={selected.position || "lower-middle"} options={[{ value: "top", label: "Top" }, { value: "center", label: "Center" }, { value: "lower-middle", label: "Lower middle" }, { value: "bottom", label: "Bottom" }]} onChange={(value) => updateSelected("position", value)} ariaLabel="Position" /></label><label>Style<SelectMenu value={selected.style || "default"} options={[{ value: "default", label: "Default" }, { value: "bold", label: "Bold" }, { value: "highlight", label: "Highlight" }, { value: "minimal", label: "Minimal" }]} onChange={(value) => updateSelected("style", value)} ariaLabel="Style" /></label></div><label>Emphasis<SelectMenu value={selected.emphasis || "none"} options={[{ value: "none", label: "None" }, { value: "keywords", label: "Keywords" }, { value: "all", label: "All words" }]} onChange={(value) => updateSelected("emphasis", value)} ariaLabel="Emphasis" /></label></>}
            {selected.type === "overlay" && <><label>Overlay text<textarea rows="3" value={selected.text || ""} onChange={(event) => updateSelected("text", event.target.value)} /></label><label>Position<SelectMenu value={selected.position || "center"} options={[{ value: "top", label: "Top" }, { value: "center", label: "Center" }, { value: "bottom", label: "Bottom" }]} onChange={(value) => updateSelected("position", value)} ariaLabel="Position" /></label></>}
      ...
      <section className="editor-timeline-card">
        <div className="editor-timeline-toolbar"><div><strong>Timeline</strong><span>{formatTime(duration)} · {timeline?.fps || FPS} fps</span></div><div className="editor-timeline-toolbar__controls"><label>Zoom<input type="range" min="50" max="180" step="5" value={zoom} onChange={(event) => setZoom(Number(event.target.value))} /></label><span>{Math.round(zoom)} px/s</span><SelectMenu className="editor-timeline-snap" value={snapMode} options={[{ value: "grid", label: "Snap grid" }, { value: "frame", label: "Snap frame" }, { value: "free", label: "Free" }]} onChange={setSnapMode} ariaLabel="Snap mode" /></div></div>
