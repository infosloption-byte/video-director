...
const RESEARCH_TABS = [
  ["overview", "Overview", "Findings and the research story"],
  ["evidence", "Evidence", "Claims, passages and verification"],
  ["sources", "Sources", "Source quality and reading status"],
  ["review", "Review", "Conflicts, reliability and research memory"],
  ["creative", "Creative", "Safe claims, gaps and story angles"],
];
  ...
  return (
    <div className="hx-page">
      ...
      <main className="container">
        ...
        {activity.length > 0 && researchStatus !== "ready" && <section className="research-brief__activity research-brief__activity--live" aria-label="Live research activity"><div><p className="eyebrow">Live research stream</p><h3>What Helix is doing now</h3></div><div role="log" aria-live="polite">{activity.slice(-8).reverse().map((item) => <article key={item.id}><span>{activityLabel(item)}</span><strong>{formatActivityDetail(item)}</strong><small>{new Date(item.at).toLocaleTimeString()}</small></article>)}</div></section>}

        {researchStatus === "ready" && activity.length > 0 && <details className="research-brief__activity-history"><summary><span><span className="eyebrow">Research activity</span><strong>Evidence pipeline history</strong></span><span>{activity.length} events</span></summary><div className="research-brief__activity" role="log">{activity.slice(-12).reverse().map((item) => <article key={item.id}><span>{activityLabel(item)}</span><strong>{formatActivityDetail(item)}</strong><small>{new Date(item.at).toLocaleTimeString()}</small></article>)}</div></details>}
        ...
        {researchStatus === "ready" && research && <section className="research-brief research-brief--deep" aria-labelledby="research-brief-title">
          ...
          <div className="research-brief__story-grid">{[["What happened", deep.what_happened], ["Why it matters", deep.why_it_matters], ["How it works", deep.mechanism]].map(([title, text]) => text && <article key={title} className="research-brief__panel"><p className="eyebrow">{title}</p><p>{text}</p></article>)}</div>
          ...
          <div className="research-brief__tab-content">
            {activeTab === "overview" && <section aria-labelledby="findings-title"><div className="research-brief__section-head"><div><p className="eyebrow">Interactive evidence view</p><h3 id="findings-title">Key findings</h3></div><span>{findings.length} findings</span></div>{findings.length > 0 ? <div className="research-brief__finding-list research-brief__finding-list--grid">{findings.map((item, index) => { const claim = verifiedClaims.find((candidate) => normalizeClaimText(candidate.claimText) === normalizeClaimText(item.claim)); const verified = Number(claim?.verifiedConfidence ?? item.confidence ?? 0); const evidenceCount = claim?.evidenceLinks?.length ?? item.evidence_indexes?.length ?? 0; return <button type="button" key={`${item.claim}-${index}`} className={`research-brief__finding-card${selectedFinding === item ? " research-brief__finding-card--selected" : ""}`} onClick={() => { setSelectedFinding(item); setActiveTab("evidence"); }} aria-pressed={selectedFinding === item}><div className="research-brief__finding-card-head"><span className={`research-brief__level research-brief__level--${item.evidence_level}`}>{item.evidence_level}</span><span className="research-brief__finding-action">Inspect →</span></div><strong>{item.claim}</strong><p>{item.evidence}</p><div className="research-brief__finding-confidence"><div><span>Confidence</span><strong>{verified}%</strong></div><div><span>Evidence</span><strong>{evidenceCount}</strong></div></div><div className="research-brief__confidence-bar" aria-hidden="true"><span style={{ width: `${Math.max(0, Math.min(100, verified))}%` }} /></div></button>; })}</div> : <p className="research-brief__empty">No findings were returned for this research run.</p>}</section>}

            {activeTab === "evidence" && <section aria-labelledby="evidence-title"><div className="research-brief__section-head"><div><p className="eyebrow">Traceability</p><h3 id="evidence-title">Claims & exact evidence</h3></div><span>{verifiedClaims.length} claims</span></div>{selectedFinding && <ResearchFindingInspector finding={selectedFinding} claim={inspectedClaim} evidence={selectedEvidence} sources={selectedSources} onClose={() => setSelectedFinding(null)} />}{!selectedFinding && <div className="research-brief__empty-state"><strong>Select a finding from Overview</strong><span>Helix will show the exact supporting passages, confidence and source comparison here.</span></div>}{numbers.length > 0 && <div className="research-brief__subsection"><div className="research-brief__section-head"><div><p className="eyebrow">Data</p><h3>Important numbers</h3></div></div><div className="research-brief__data-grid">{numbers.map((item, index) => <article key={`${item.value}-${index}`}><strong>{item.value}</strong><span>{item.context}</span></article>)}</div></div>}{verifiedClaims.length > 0 && <div className="research-brief__subsection"><div className="research-brief__finding-list research-brief__finding-list--grid">{verifiedClaims.map((claim) => <article key={claim.id}><div><VerificationBadge claim={claim} /><strong>{claim.claimText}</strong></div><small>{claim.verifiedConfidence ?? 0}% verified · {claim.verification?.authorityScore ?? 0}% authority · {claim.verification?.corroborationScore ?? 0}% corroboration</small></article>)}</div></div>}</section>}

            {activeTab === "sources" && <section aria-labelledby="sources-title"><div className="research-brief__section-head"><div><p className="eyebrow">Source inspection</p><h3 id="sources-title">Evidence source quality</h3></div><span>{graphSources.length} sources</span></div>{graphSources.length > 0 && <div className="research-brief__data-grid research-brief__source-grid">{graphSources.slice(0, 12).map((source) => <article key={source.id}><div className="research-brief__source-title"><strong>{source.title || source.publisher || "Source"}</strong><span>{source.sourceClass || source.reliability || "Source"}</span></div><div className="research-brief__source-score"><strong>{source.authorityScore ?? 0}%</strong><span>authority</span></div><small>{source.readStatus}</small><SourceLink source={source} /></article>)}</div>}{research.sources?.length > 0 && <div className="research-brief__subsection"><div className="research-brief__section-head"><div><p className="eyebrow">Discovery</p><h3>Source library</h3></div></div><div className="research-brief__source-library">{research.sources.map((source, index) => <article key={`${source.url}-${index}`}><div><strong>{source.title}</strong><span>{source.note}</span></div><small>{source.source_class || source.source_reliability} · quality prior {source.quality_prior ?? "—"}</small><SourceLink source={source} /></article>)}</div></div>}</section>}

            {activeTab === "review" && <section aria-labelledby="review-title"><div className="research-brief__section-head"><div><p className="eyebrow">Cross-checking</p><h3 id="review-title">Review research integrity</h3></div><span>{graphConflicts.length} conflict cases</span></div>{graphConflicts.length > 0 && <div className="research-brief__conflict-list">{graphConflicts.map((item) => <article key={item.id}><div><span className="research-brief__verification">{item.status}</span><strong>{item.overlapScore}% claim overlap</strong></div><p>{item.reason}</p><small>{item.resolution || "Resolution is available in the research memory workspace."}</small></article>)}</div>}{disagreements.length > 0 && <div className="research-brief__subsection"><div className="research-brief__section-head"><div><p className="eyebrow">Conflicting evidence</p><h3>Where sources disagree</h3></div></div><div className="research-brief__conflict-list">{disagreements.map((item, index) => <article key={`${item.topic}-${index}`}><strong>{item.topic}</strong><p>{item.positions?.join(" / ")}</p><small>{item.resolution} · {item.confidence}% confidence</small></article>)}</div></div>}{reliability && <div className="research-brief__panel research-brief__panel--reliability"><p className="eyebrow">Reliability assessment</p><h3>How much to trust this brief</h3><p>{reliability.rationale}</p>{reliability.limitations?.length > 0 && <ul>{reliability.limitations.map((item, index) => <li key={index}>{item}</li>)}</ul>}</div>}<ResearchWorkspacePanels projectId={id} conflicts={graphConflicts} onRefresh={() => loadGraph().catch(() => {})} /></section>}

            {activeTab === "creative" && <section aria-labelledby="creative-title"><div className="research-brief__section-head"><div><p className="eyebrow">Downstream video guidance</p><h3 id="creative-title">Turn research into a story</h3></div></div><div className="research-brief__grid">{safeClaims.length > 0 && <article className="research-brief__panel research-brief__panel--safe"><p className="eyebrow">Use confidently</p><h3>Claims safe to say</h3><ul>{safeClaims.map((item, index) => <li key={index}>{item}</li>)}</ul></article>}{avoidClaims.length > 0 && <article className="research-brief__panel research-brief__panel--avoid"><p className="eyebrow">Avoid overclaiming</p><h3>Claims to avoid</h3><ul>{avoidClaims.map((item, index) => <li key={index}>{item}</li>)}</ul></article>}</div>{gaps.length > 0 && <div className="research-brief__subsection"><div className="research-brief__section-head"><div><p className="eyebrow">Still uncertain</p><h3>Knowledge gaps</h3></div></div><div className="research-brief__compact-list">{gaps.map((item, index) => <div key={index}>{item}</div>)}</div></div>}{opportunities.length > 0 && <div className="research-brief__panel research-brief__creative-angle"><p className="eyebrow">Creative opportunities</p><h3>Recommended story angle</h3><p>{deep.recommended_story_angle}</p><ul>{opportunities.map((item, index) => <li key={index}>{item}</li>)}</ul></div>}</section>}
          </div>
