...
export default function ResearchPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/projects/${id}/research/graph`, { cache: "no-store" })
      .then(async (response) => { const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data.error || "Failed to load research evidence."); if (!stopped) setGraph(data.session || null); })
  ...
  return (
    <div className="hx-page">
      ...
      <main className="container">
        ...
        {activity.length > 0 && researchStatus !== "ready" && (
          <section className="research-brief__activity" aria-label="Live research activity">
            <div><p className="eyebrow">Live research stream</p><h3>What Helix is doing now</h3></div>
            <div role="log" aria-live="polite">{activity.slice(-12).reverse().map((item) => <article key={item.id}><span>{activityLabel(item)}</span><strong>{formatActivityDetail(item)}</strong><small>{new Date(item.at).toLocaleTimeString()}</small></article>)}</div>
          </section>
        )}
        {researchStatus === "ready" && activity.length > 0 && (
          <section className="research-brief__activity" aria-label="Research activity history">
            <div><p className="eyebrow">Research activity</p><h3>Evidence pipeline history</h3></div>
            <div role="log">{activity.slice(-12).reverse().map((item) => <article key={item.id}><span>{activityLabel(item)}</span><strong>{formatActivityDetail(item)}</strong><small>{new Date(item.at).toLocaleTimeString()}</small></article>)}</div>
          </section>
        )}
