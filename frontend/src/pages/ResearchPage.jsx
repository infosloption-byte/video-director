...
export default function ResearchPage() {
  ...
  // publicProject already exposes the complete persisted brief under research.sources.
  ...
  return (
    <div className="hx-page">
      ...
      <main className="container">
        ...
        {researchStatus === "ready" && research && <div className="research-workspace-layout">
          <section className="research-brief research-brief--deep" aria-labelledby="research-brief-title">
            ...
            <div className="research-brief__tab-content">
              {activeTab === "overview" && <section aria-labelledby="findings-title">
                {[["Executive synthesis", deep.executive_summary], ["What happened", deep.what_happened], ["Why it matters", deep.why_it_matters], ["How it works", deep.mechanism]].filter(([, text]) => text).map(([title, text]) => <article key={title} className="research-brief__panel research-brief__synthesis-panel"><p className="eyebrow">{title}</p><p>{text}</p></article>)}
                <div className="research-brief__section-head"><div><p className="eyebrow">Interactive evidence view</p><h3 id="findings-title">Key findings</h3></div><span>{findings.length} findings</span></div>
                {findings.length > 0 ? <div className="research-brief__finding-list research-brief__finding-list--grid">{findings.map((item, index) => { const claim = verifiedClaims.find((candidate) => normalizeClaimText(candidate.claimText) === normalizeClaimText(item.claim)); const verified = Number(claim?.verifiedConfidence ?? item.confidence ?? 0); const evidenceCount = claim?.evidenceLinks?.length ?? item.evidence_indexes?.length ?? 0; return <button type="button" key={`${item.claim}-${index}`} className={`research-brief__finding-card${selectedFinding === item ? " research-brief__finding-card--selected" : ""}`} onClick={() => { setSelectedFinding(item); setActiveTab("evidence"); }} aria-pressed={selectedFinding === item}><div className="research-brief__finding-card-head"><span className={`research-brief__level research-brief__level--${item.evidence_level}`}>{item.evidence_level}</span><span className="research-brief__finding-action">Inspect →</span></div><strong>{item.claim}</strong><p>{item.evidence}</p><div className="research-brief__finding-confidence"><div><span>Confidence</span><strong>{verified}%</strong></div><div><span>Evidence</span><strong>{evidenceCount}</strong></div></div><div className="research-brief__confidence-bar" aria-hidden="true"><span style={{ width: `${Math.max(0, Math.min(100, verified))}%` }} /></div></button>; })}</div> : <p className="research-brief__empty">No key findings were returned in the saved research brief.</p>}
                {numbers.length > 0 && <div className="research-brief__subsection"><div className="research-brief__section-head"><div><p className="eyebrow">Data</p><h3>Important numbers</h3></div><span>{numbers.length} data points</span></div><div className="research-brief__data-grid">{numbers.map((item, index) => <article key={`${item.value}-${index}`}><strong>{item.value}</strong><span>{item.context}</span></article>)}</div></div>}
              </section>}
