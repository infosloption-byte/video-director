...
export default function ProjectProductivityPage() {
  ...
  const refresh = useCallback(async () => {
    try {
      const [versionData, templateData, activityData, reviewData] = await Promise.all([
        api(`/api/projects/${id}/versions`),
        api("/api/templates"),
        api(`/api/projects/${id}/activity`),
        api(`/api/projects/${id}/review-links`),
      ]);
      setVersions(versionData.versions || []); setTemplates(templateData.templates || []); setActivity(activityData.activity || []); setLinks(reviewData.reviews || []);
    } catch (loadError) {
  ...
  // The state updates occur asynchronously after the API boundary.
  ...
  async function applyTemplate(template) { setBusy(true); setError(""); try { const editor = await api(`/api/projects/${id}/editor`); await api(`/api/projects/${id}/apply-template/${template.id}`, { method: "POST", body: JSON.stringify({ version: editor.editor.version }) }); setMessage(`Template applied: ${template.name}`); await refresh(); } catch (e) { setError(e.message); } finally { setBusy(false); } }
  async function deleteTemplate(template) { setBusy(true); setError(""); try { await api(`/api/templates/${template.id}`, { method: "DELETE" }); setMessage(`Template deleted: ${template.name}`); await refresh(); } catch (e) { setError(e.message); } finally { setBusy(false); } }
  ...
  async function revokeReviewLink(link) { setBusy(true); setError(""); try { await api(`/api/projects/${id}/review-links/${link.id}`, { method: "DELETE" }); setMessage("Review link revoked."); await refresh(); } catch (e) { setError(e.message); } finally { setBusy(false); } }
  ...
  return <div className="hx-page"><Header right={<Link to={`/editor/${id}`} className="btn btn-ghost">Back to editor</Link>} /><main className="container" style={{ maxWidth: 1000, paddingTop: 40, paddingBottom: 64 }}>
    <p className="eyebrow">Workspace · productivity</p><h1>Versions, templates & review</h1><p>Save named editor snapshots, restore safely, duplicate projects, reuse editor templates, and create managed review links.</p>
    ...
    <section className="hx-card" style={{ marginTop: 16, padding: 24 }}><h2>Templates</h2><div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}><input value={templateName} onChange={(e) => setTemplateName(e.target.value)} placeholder="Template name" /><button className="btn btn-cream" disabled={busy || !templateName.trim()} onClick={createTemplate}>Save current timeline</button></div><div style={{ display: "grid", gap: 8, marginTop: 16 }}>{templates.map((template) => <div key={template.id} style={{ padding: 10, border: "1px solid var(--border-color, #ddd)", borderRadius: 8, display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center" }}><strong>{template.name}</strong><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><button className="btn btn-ghost" disabled={busy} onClick={() => applyTemplate(template)}>Apply</button><button className="btn btn-ghost" disabled={busy} onClick={() => deleteTemplate(template)}>Delete</button></div></div>)}{!templates.length && <p>No templates yet.</p>}</div></section>
    <section className="hx-card" style={{ marginTop: 16, padding: 24 }}><h2>Review</h2><button className="btn btn-cream" disabled={busy} onClick={createReviewLink}>Create review link</button><div style={{ display: "grid", gap: 8, marginTop: 16 }}>{links.map((link) => <div key={link.id} style={{ border: "1px solid var(--border-color, #ddd)", borderRadius: 8, padding: 12 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center" }}><strong>{link.revokedAt ? "Revoked" : "Active"}</strong><button className="btn btn-ghost" disabled={busy || Boolean(link.revokedAt)} onClick={() => revokeReviewLink(link)}>Revoke</button></div><p style={{ wordBreak: "break-all", margin: "8px 0" }}>{window.location.origin}/review/{link.token}</p><small>{link._count?.comments || 0} comments{link.expiresAt ? ` · expires ${new Date(link.expiresAt).toLocaleString()}` : " · no expiry"}</small></div>)}{!links.length && <p>No review links yet.</p>}</div><p style={{ fontSize: 13, opacity: .7, marginTop: 12 }}>Review links expose the current editor timeline read-only and allow reviewers to leave comments.</p></section>
