...
export default function MediaLibraryPage() {
  ...
  const [musicQuery, setMusicQuery] = useState("ambient cinematic");
  ...
  async function searchExternalMusic(event) {
    event?.preventDefault();
    const normalized = String(musicQuery || "").trim();
    if (!normalized) return;
    setSearchingMusic(true);
    setMessage("");
    try {
      const response = await fetch(`/api/projects/${id}/media/music-search?query=${encodeURIComponent(normalized)}&limit=12`, { credentials: "include", cache: "no-store" });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data.error || "External music search failed.");
      setMusicResults(data.results || []);
    } catch (error) {
      setMusicResults([]);
      setMessage(error.message || "External music search failed.");
    } finally {
      setSearchingMusic(false);
    }
  }

  async function importExternalMusic(result) {
    if (!result?.audioUrl) return;
    const key = result.providerAssetId || result.audioUrl;
    setSavingId(key);
    setMessage("");
    try {
      const response = await fetch(`/api/projects/${id}/media`, {
        ...
        body: JSON.stringify({
          kind: "audio", origin: "external", title: result.title || "Music",
          mediaUrl: result.audioUrl, thumbnailUrl: result.thumbnailUrl, sourceUrl: result.sourceUrl,
          provider: result.provider || "jamendo", providerAssetId: result.providerAssetId,
          durationSeconds: result.durationSeconds,
        }),
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data.error || "Failed to add music.");
      setMedia((items) => [data.media, ...items.filter((item) => item.id !== data.media.id)]);
      setMessage(data.created ? "Music added to project library." : "Music is already in the project library.");
    } catch (error) {
      setMessage(error.message || "Failed to add music.");
    } finally {
      setSavingId("");
    }
  }
  ...
  return <div className="hx-page">
    ...
    <main className="container media-library">
      ...
      <section className="media-library__search-card">
        <div className="editor-section-title"><strong>Music library</strong><span>Search external music · Jamendo</span></div>
        <form className="media-library__search" onSubmit={searchExternalMusic}>
          <input aria-label="Search external music" value={musicQuery} onChange={(event) => setMusicQuery(event.target.value)} placeholder="Search mood, genre, instrument" />
          <button className="btn btn-cream" type="submit" disabled={searchingMusic}>{searchingMusic ? "Searching…" : "Search music"}</button>
        </form>
        {musicResults.length > 0 && <div className="media-library__results">{musicResults.map((result) => <article className="media-result" key={result.providerAssetId || result.audioUrl}>
          <div className="media-result__thumb">{result.thumbnailUrl ? <img src={result.thumbnailUrl} alt="" /> : <div className="media-card__placeholder">music</div>}</div>
          <div className="media-result__body"><strong>{result.title || "Music"}</strong><span>{result.artist || "Jamendo artist"}{result.durationSeconds ? ` · ${Number(result.durationSeconds).toFixed(0)}s` : ""}</span><audio src={result.audioUrl} controls preload="none" /><button className="btn btn-ghost" type="button" disabled={savingId === (result.providerAssetId || result.audioUrl)} onClick={() => void importExternalMusic(result)}>{savingId === (result.providerAssetId || result.audioUrl) ? "Adding…" : "Add to library"}</button></div>
        </article>)}</div>}
      </section>
