...
function statusLabel(status) {
  return ({ queued: "Starting research", planning: "Planning research", discovering: "Discovering sources", reading: "Reading sources", verifying: "Verifying evidence", synthesizing: "Building research brief", ready: "Research ready", error: "Research needs attention" })[status] || "Researching";
}
...
export default function ResearchConversationPage() {
  ...
  useEffect(() => { let cancelled = false; load().catch((err) => { if (!cancelled) { setError(err.message); setLoading(false); } }); return () => { cancelled = true; }; }, [load]);
  useEffect(() => { if (!project || TERMINAL.has(project.researchStatus)) return undefined; const timer = window.setInterval(() => { load().catch(() => {}); }, 1800); return () => window.clearInterval(timer); }, [project, load]);
  ...
  async function sendQuestion(event) {
    ...
    setSending(true); setError(""); setMessages((current) => [...current, { role: "user", content: text, optimistic: true }]); setQuestion("");
    try {
