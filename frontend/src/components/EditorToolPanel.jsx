...
function MediaPanel({ id, selectedClip, onReplace, onClose }) {
  const [media, setMedia] = useState([]); const [filter, setFilter] = useState("all"); const [query, setQuery] = useState(""); const [musicResults, setMusicResults] = useState([]); const [musicQuery, setMusicQuery] = useState("ambient cinematic"); const [busy, setBusy] = useState(""); const [message, setMessage] = useState("");
  ...
  async function upload(file) {
    ...
    if (!kind) { setMessage("Choose a video, image, or audio file."); return; }
    setBusy("upload"); setMessage("");
    try {
      const params = new URLSearchParams({ kind, filename: file.name });
      const response = await fetch(`/api/projects/${id}/media/upload?${params}`, { method: "PUT", credentials: "include", headers: { "Content-Type": file.type || "application/octet-stream", "X-Requested-With": "XMLHttpRequest" }, body: file });
      const data = await response.json().catch(() => ({}));
  ...
  async function removeMedia(item) {
    ...
    try {
      const response = await fetch(`/api/projects/${id}/media/${item.id}`, { method: "DELETE", credentials: "include", headers: { "X-Requested-With": "XMLHttpRequest" } });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data.error || "Failed to remove media.");
      setMedia((items) => items.filter((entry) => entry.id !== item.id));
    } catch (err) { setMessage(err.message); }
    finally { setBusy(""); }
  }
  return <Panel title="Media Library" eyebrow="Editor tool" onClose={onClose}>
    <div className="editor-tool-tabs">{[["all","All"],["video","Video"],["image","Image"],["audio","Audio"]].map(([value,label]) => <button type="button" key={value} className={filter === value ? "is-active" : ""} onClick={() => setFilter(value)}>{label}</button>)}</div>
    <div className="editor-tool-search editor-media-search"><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search project media" /></div>
    <label className={`editor-tool-upload ${busy === "upload" ? "is-uploading" : ""}`}>
      <input type="file" accept="video/*,image/*,audio/*" disabled={Boolean(busy)} onChange={(e) => { const selected = e.target.files?.[0]; e.target.value = ""; void upload(selected); }} />
      <span className="editor-tool-upload__copy"><strong>{busy === "upload" ? "Uploading…" : "Drop video, image or audio here"}</strong><small>or click anywhere here to browse files</small></span>
      <span className="editor-tool-upload__icon" aria-hidden="true">↑</span>
    </label>
    {message && <p className="editor-tool-message" role="status">{message}</p>}
    <div className="editor-tool-grid">{visible.map((item) => <article key={item.id} className="editor-media-item">
      <div className="editor-media-item__preview">{item.kind === "video" ? <video src={item.proxyUrl || item.mediaUrl} poster={item.thumbnailUrl || undefined} muted playsInline preload="metadata" /> : item.kind === "image" ? <img src={item.thumbnailUrl || item.mediaUrl} alt="" /> : <span>♫</span>}</div>
      <div className="editor-media-item__info"><strong title={item.title || item.filename || "Untitled"}>{item.title || item.filename || "Untitled"}</strong><small>{item.kind}{item.durationSeconds ? ` · ${Number(item.durationSeconds).toFixed(1)}s` : ""}</small></div>
      <button type="button" className="editor-media-item__delete" onClick={() => void removeMedia(item)} disabled={Boolean(busy)} aria-label={`Delete ${item.title || item.filename || "media"}`} title="Remove from project library">⌫</button>
    </article>)}</div>
    <div className="editor-tool-section"><h3>Music search</h3><form className="editor-tool-search" onSubmit={searchMusic}><input value={musicQuery} onChange={(e) => setMusicQuery(e.target.value)} /><button type="submit" className="btn btn-ghost" disabled={busy === "music-search"}>{busy === "music-search" ? "Searching…" : "Search"}</button></form>{musicResults.map((item) => <div className="editor-tool-row" key={item.providerAssetId || item.audioUrl}><div><strong>{item.title || "Music"}</strong><span>{item.durationSeconds ? `${Number(item.durationSeconds).toFixed(1)}s` : ""}</span></div><button type="button" className="btn btn-ghost" disabled={Boolean(busy)} onClick={() => void importMusic(item)}>Import</button></div>)}</div>
  </Panel>;

  async function importMusic(item) {
    const key = item.providerAssetId || item.audioUrl;
    if (!key) return;
    setBusy(key);
    try {
      const data = await api(`/api/projects/${id}/media`, { method: "POST", body: JSON.stringify({ kind: "audio", origin: "external", title: item.title || "Music", mediaUrl: item.audioUrl, thumbnailUrl: item.thumbnailUrl, sourceUrl: item.sourceUrl, provider: item.provider || "jamendo", providerAssetId: item.providerAssetId, durationSeconds: item.durationSeconds }) });
      setMedia((items) => [data.media, ...items.filter((x) => x.id !== data.media.id)]);
    } catch (err) { setMessage(err.message); }
    finally { setBusy(""); }
  }
}
...
function AIPanel({ id, onClose, onApplied }) {
  ...
  return <Panel title="AI Assistance" eyebrow="Editor tool" onClose={onClose}>
    ...
    <div className="editor-intent-list">{INTENTS.map(([label,text]) => <button type="button" key={label} onClick={() => { setInstruction(text); setSuggestion(null); setPreview(null); }}>{label}</button>)}</div>
    ...
    <div className="editor-tool-actions"><button type="button" className="btn btn-cream" disabled={busy || !instruction.trim()} onClick={suggest}>{busy ? "Working…" : "Suggest edits"}</button>{suggestion && <button type="button" className="btn btn-ghost" disabled={busy} onClick={previewSuggestion}>Preview</button>}{preview && <button type="button" className="btn btn-cream" disabled={busy} onClick={apply}>Apply</button>}{undoTimeline && <button type="button" className="btn btn-ghost" disabled={busy} onClick={undo}>Undo AI</button>}</div>
...
function RenderPanel({ id, onClose }) {
  ...
  return <Panel title="Render Video" eyebrow="Editor tool" onClose={onClose}>
    ...
    <div className="editor-tool-actions"><button type="button" className="btn btn-cream" disabled={busy} onClick={start}>{busy ? "Rendering…" : "Render MP4"}</button><button type="button" className="btn btn-ghost" disabled={busy} onClick={refresh}>Refresh</button></div>
  </Panel>;
}
...
function ProductivityPanel({ id, onClose }) {
  ...
  return <Panel title="Productivity" eyebrow="Editor tool" onClose={onClose}>
    ...
    <div className="editor-tool-section"><h3>Versions</h3><div className="editor-tool-search"><input value={label} onChange={e=>setLabel(e.target.value)} placeholder="Snapshot label"/><button type="button" className="btn btn-cream" disabled={busy} onClick={()=>action(()=>api(`/api/projects/${id}/versions`,{method:"POST",body:JSON.stringify({label})}),"Version snapshot created.")}>Save</button></div>{versions.map(v=><div className="editor-tool-row" key={v.id}><div><strong>v{v.versionNumber} · {v.label}</strong><span>{new Date(v.createdAt).toLocaleString()}</span></div><button type="button" className="btn btn-ghost" disabled={busy} onClick={()=>action(async()=>{const e=await api(`/api/projects/${id}/editor`);await api(`/api/projects/${id}/restore/${v.id}`,{method:"POST",body:JSON.stringify({version:e.editor.version})});},`Restored ${v.label}.`)}>Restore</button></div>)}</div>
    <div className="editor-tool-section"><h3>Templates</h3><div className="editor-tool-search"><input value={templateName} onChange={e=>setTemplateName(e.target.value)} placeholder="Template name"/><button type="button" className="btn btn-cream" disabled={busy||!templateName.trim()} onClick={()=>action(()=>api(`/api/projects/${id}/template`,{method:"POST",body:JSON.stringify({name:templateName})}),"Template saved.")}>Save</button></div>{templates.map(t=><div className="editor-tool-row" key={t.id}><strong>{t.name}</strong><div><button type="button" className="btn btn-ghost" disabled={busy} onClick={()=>action(async()=>{const e=await api(`/api/projects/${id}/editor`);await api(`/api/projects/${id}/apply-template/${t.id}`,{method:"POST",body:JSON.stringify({version:e.editor.version})});},`Template applied: ${t.name}`)}>Apply</button><button type="button" className="btn btn-ghost" disabled={busy} onClick={()=>action(()=>api(`/api/templates/${t.id}`,{method:"DELETE"}),`Template deleted: ${t.name}`)}>Delete</button></div></div>)}</div>
    <div className="editor-tool-section"><h3>Review</h3><button type="button" className="btn btn-cream" disabled={busy} onClick={()=>action(async()=>{const d=await api(`/api/projects/${id}/review-links`,{method:"POST",body:JSON.stringify({})});setMessage(`Review link created: ${window.location.origin}/review/${d.review.token}`);},"Review link created.")}>Create review link</button>{links.map(l=><div className="editor-tool-row" key={l.id}><div><strong>{l.revokedAt?"Revoked":"Active"}</strong><span>{l._count?.comments||0} comments</span></div><button type="button" className="btn btn-ghost" disabled={busy||Boolean(l.revokedAt)} onClick={()=>action(()=>api(`/api/projects/${id}/review-links/${l.id}`,{method:"DELETE"}),"Review link revoked.")}>Revoke</button></div>)}</div>
