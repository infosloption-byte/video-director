...
export default function PhonePreview({ step, duration, cuts, playing, onTogglePlay }) {
  ...
  const words = String(step.line || "").split(" ");
    ...
    <div className="hx-preview">
      <div className="hx-preview__frame" style={{ background }}>
        ...
        <div className="hx-preview__captions">
          ...
          <p className="hx-preview__caption-sub"><span className="is-said">{leadWord}</span>{" "}<span className="is-pending">{restWords}</span></p>
        </div>
        ...
        <div className="hx-preview__controls">
          ...
          <div className="hx-preview__scrub"><div className="hx-preview__track"><div className="hx-preview__fill" style={{ width: playing ? "4%" : "0%" }} /></div></div>
          <span className="mono-label hx-preview__time">{playing ? "00:01" : "00:00"} / {duration}</span>
        </div>
      </div>
      <p className="hx-preview__note">9:16 safe zone · captions upper-middle · {cuts} cuts</p>
    </div>
  );
}
